...
fn test_monitor_bar_content_generation() {
    ...
    let content = format!(
        "Niri Bar - {} ({}x{}, scale={})", 
        monitor.connector, 
        monitor.logical_size.0, 
        monitor.logical_size.1,
...
fn test_monitor_change_detection_logic() {
    ...
    let new_monitors = vec!["eDP-1".to_string(), "HDMI-A-1".to_string()];
...
fn test_monitor_bar_styling_consistency() {
    ...
    for monitor in monitors {
        ...
        let css_class = format!("monitor-bar-{}", monitor.connector.to_lowercase().replace('-', "_"));
...
fn test_monitor_scale_factor_calculation() {
    ...
    for (physical_w, physical_h, scale, expected_logical_w, expected_logical_h) in test_cases {
